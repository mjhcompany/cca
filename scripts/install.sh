...
if [[ ! -f "$RELEASE_DIR/cca" ]] || [[ ! -f "$RELEASE_DIR/ccad" ]]; then
    echo -e "${RED}Error: Release binaries not found.${NC}"
    echo "Please build first with: cargo build --release --workspace"
    exit 1
fi
echo -e "  ${GREEN}✓${NC} Release binaries found"
...
# =============================================================================
if [[ "$START_SERVICES" == "yes" ]] && [[ "$SKIP_DOCKER" != "true" ]]; then
    echo ""
    ...
    echo ""
    echo -e "${BLUE}Docker services already running, skipping startup...${NC}"
fi
...
echo ""
echo "Next steps:"
echo "  1. Source environment (add to ~/.bashrc for persistence):"
echo "     source ${CONFIG_DIR}/cca.env"
echo ""
echo "  2. Start the CCA daemon:"
echo "     ccad"
echo ""
echo "  3. Or use the CLI:"
echo "     cca daemon start"
echo ""
